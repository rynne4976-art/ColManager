...
<div class="container-fluid px-4 mt-4">
    ...
    <c:if test="${not empty attendanceMessage}">
        <div class="alert 
>>>>>>> 9b625ce9cd8955722639872d004e169459a31803
            <c:choose>
			...
			${attendanceMessage}</div>
	</c:if>

	<div class="card mb-4">
		<div class="card-header">
			<i class="fas fa-calendar-check me-1"></i> 출석 조회 조건
		</div>
		<div class="card-body">
			<form
				action="${pageContext.request.contextPath}/classroom/attendanceProfessor.do"
				method="get">
				<div class="row g-3 align-items-end">
					<div class="col-md-3">
						<label class="form-label">과목 선택</label> <select name="course_id"
							class="form-select" required>
							<option value="">과목을 선택하세요</option>
							<c:forEach var="course" items="${professorCourseList}">
								<option value="${course.course_id}"
									<c:if test="${selectedCourseId == course.course_id}">selected</c:if>>
									${course.course_name} (${course.course_id})</option>
							</c:forEach>
						</select>
					</div>

					<div class="col-md-2">
						<label class="form-label">수업 날짜</label> <input type="date"
							name="class_date" value="${classDate}" class="form-control"
							required>
					</div>

					<div class="col-md-2">
						<label class="form-label">학생명 검색</label> <input type="text"
							name="student_name" value="${studentNameKeyword}"
							class="form-control" placeholder="학생명을 입력하세요">
					</div>

					<div class="col-md-2">
						<label class="form-label">출석상태</label> <select
							name="attendance_status" class="form-select">
							<option value="">전체</option>
							<option value="출석"
								<c:if test="${attendanceStatusFilter == '출석'}">selected</c:if>>출석</option>
							<option value="지각"
								<c:if test="${attendanceStatusFilter == '지각'}">selected</c:if>>지각</option>
							<option value="결석"
								<c:if test="${attendanceStatusFilter == '결석'}">selected</c:if>>결석</option>
							<option value="공결"
								<c:if test="${attendanceStatusFilter == '공결'}">selected</c:if>>공결</option>
							<option value="미등록"
								<c:if test="${attendanceStatusFilter == '미등록'}">selected</c:if>>미등록</option>
						</select>
					</div>
				...
				<div class="row mt-3">
					...
					<div class="col-md-8 d-flex justify-content-end">
						<a
							href="${pageContext.request.contextPath}/classroom/attendanceProfessor.do"
							class="btn btn-outline-secondary"> 필터 초기화 </a>
					</div>
				</div>
			</form>
		</div>
	...
	<c:if test="${not empty studentList}">
		<div class="card">
			<div class="card-header">
				<i class="fas fa-user-check me-1"></i> 학생별 출석 입력
			</div>
			<div class="card-body">
				<form id="attendanceForm"
					...
					<input type="hidden" name="student_name"
						value="${studentNameKeyword}"> <input type="hidden"
						name="attendance_status" value="${attendanceStatusFilter}">
					<input type="hidden" name="saveMode" id="saveMode"> <input
						type="hidden" name="single_student_id" id="singleStudentId">
					...
					<div class="row g-2 mb-3">
						...
						<div class="col-md-2">
							<button type="button" id="btnApplySelected"
								class="btn btn-outline-primary w-100"
								onclick="applySelectedStatus()">선택 적용</button>
						</div>

						<div class="col-md-2">
							<button type="button" id="btnApplyAll"
								class="btn btn-outline-secondary w-100"
								onclick="applyAllStatus()">전체 적용</button>
						</div>

						<div class="col-md-2">
							<button type="button" id="btnClearChecks"
								class="btn btn-outline-dark w-100" onclick="clearAllChecks()">
								전체 해제</button>
						</div>
					</div>
						...
						<table
							...
							<thead class="table-light">
								<tr>
									<th style="width: 7%;"><input type="checkbox"
										id="checkAll" onclick="toggleAllStudents(this)"></th>
							...
							<tbody>
								<c:forEach var="student" items="${studentList}">
									...
									<tr
										class="<c:if test='${not empty savedAtt}'>table-light</c:if>">
										<td><input type="checkbox" class="student-check"
											name="student_id" value="${student.student_id}"
											<c:if test="${not empty savedAtt}">disabled</c:if>></td>
										<td>${student.student_id}</td>
										<td>${student.user_name}</td>
										<td><select name="status_${student.student_id}"
											class="form-select"
											<c:if test="${not empty savedAtt}">disabled</c:if>>
												<option value="">선택</option>
												<option value="출석"
													<c:if test="${savedAtt.status == '출석'}">selected</c:if>>출석</option>
												<option value="지각"
													<c:if test="${savedAtt.status == '지각'}">selected</c:if>>지각</option>
												<option value="결석"
													<c:if test="${savedAtt.status == '결석'}">selected</c:if>>결석</option>
												<option value="공결"
													<c:if test="${savedAtt.status == '공결'}">selected</c:if>>공결</option>
										</select></td>
										<td><input type="text"
											name="remark_${student.student_id}" class="form-control"
											value="${savedAtt.remark}" placeholder="비고 입력"
											<c:if test="${not empty savedAtt}">readonly</c:if>></td>
										<td><c:choose>

												<c:when test="${not empty savedAtt}">
													<span class="badge bg-secondary">등록완료</span>
												</c:when>
					...
					<div class="d-flex justify-content-end gap-2 mt-3">
						<button type="button" id="btnSubmitSelected"
							class="btn btn-success" onclick="submitSelectedAttendance(this)">
							선택 학생 등록</button>
					</div>
				</form>
			</div>
	...
	<c:if test="${empty studentList && not empty selectedCourseId}">
		<div class="alert alert-warning">조회 조건에 맞는 학생이 없습니다.</div>
	</c:if>
</div>
...
<script>
	...
	function getAllActionButtons() {
		return document
				.querySelectorAll("#btnApplySelected, #btnApplyAll, #btnClearChecks, #btnSubmitSelected, .action-btn-single");
	}
	...
	function applySelectedStatus() {
		...
		checks.forEach(function(chk) {
			...
			const selectBox = document.querySelector("select[name='status_"
					+ studentId + "']");
	...
	function validateStatus(studentIds) {
		for (let i = 0; i < studentIds.length; i++) {
			...
			const selectBox = document.querySelector("select[name='status_"
					+ studentId + "']");
			if (!selectBox || selectBox.disabled || selectBox.value === "") {
